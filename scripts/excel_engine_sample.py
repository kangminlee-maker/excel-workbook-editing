...
SCRIPT_PATH = Path(__file__).with_name("excel_recalculate_and_sample.applescript")
...
def sample_workbook_cells(
    ...
    *,
    ...
    By default this opens a temporary copy from Excel's own sandbox container. That
    keeps validation on the real Excel engine while reducing repeated macOS
    file-access prompts and avoiding locks on the source workbook.
    """
    if not cell_refs:
        raise ValueError("cell_refs must not be empty")
    if not SCRIPT_PATH.exists():
        raise FileNotFoundError(f"missing AppleScript helper: {SCRIPT_PATH}")
    ...
    if sandbox_copy:
        ...
        with tempfile.TemporaryDirectory(prefix="run_", dir=str(sandbox_root)) as tmp_dir:
            ...
            output = _run_applescript(tmp_path, worksheet, cell_refs, timeout)
    else:
        output = _run_applescript(workbook_path, worksheet, cell_refs, timeout)
...
def _excel_sandbox_root(sandbox_subdir: str) -> Path:
    ...
    if EXCEL_CONTAINER_DOCUMENTS.exists():
        return EXCEL_CONTAINER_DOCUMENTS / sandbox_subdir
...
def main() -> None:
    ...
    parser.add_argument(
        "--direct",
        action="store_true",
        help="Open the source workbook path directly instead of a sandbox copy.",
    )
