on run argv
    if (count of argv) < 2 then
        error "Usage: osascript excel_copy_ranges_png.applescript <workbook-path> <targets-tsv>"
    end if
            ...
            repeat with targetLine in targetLines
                ...
                if lineText is not "" then
                    ...
                    if (count of parts) is less than 4 then
                        ...
                        set pngPath to item 4 of parts

                        try
                            set ws to worksheet sheetName of wb
                            copy picture (range rangeText of ws) appearance screen format picture
                            my write_png_clipboard(pngPath)
                            set end of outputLines to targetId & tabChar & "captured" & tabChar & pngPath
...
on write_png_clipboard(pngPath)
    set pngData to the clipboard as «class PNGf»
    set outFile to open for access (POSIX file pngPath) with write permission
    try
