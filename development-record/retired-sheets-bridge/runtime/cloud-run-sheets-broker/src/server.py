...
from sheets_client import http_metadata_transport, http_values_write_transport
...
def load_runtime_config(env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    runtime_config["workload_identity"] = build_readiness_summary(runtime_config)
...
def dispatch_inspect(
    *,
    ...
    write_transport=http_values_write_transport,
    audit_sink=None,
...
def identity_authorization(headers) -> str | None:
    return headers.get("X-Broker-Authorization")
        ...
        def do_GET(self) -> None:
            ...
            self._write_json(
                200,
                {
                    ...
                    "workload_identity": runtime_config["workload_identity"],
                },
            )
