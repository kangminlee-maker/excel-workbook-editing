...
class ServerTests(unittest.TestCase):
    def test_health_returns_public_json_readiness_signal(self) -> None:
        ...
        self.assertEqual(body["service"], "cloud-run-sheets-broker")
        self.assertEqual(body["workload_identity"]["authority_mode"], "workload_identity")
        self.assertTrue(body["workload_identity"]["ready"])
        self.assertNotIn("day1-mcp-wrapper", json.dumps(body))
    ...
    def test_load_runtime_config_builds_auth_policy_and_service_account(self) -> None:
        config = load_runtime_config(
            {
                ...
                "BROKER_ADDITIONAL_AUDIENCES": "mcp-client-id, broker-client-id",
        ...
        self.assertEqual(
            ...
            ("broker-client-id", "mcp-client-id"),
        )
        ...
        self.assertTrue(config["workload_identity"]["ready"])
        self.assertEqual(
            config["workload_identity"]["token_flow"],
            [
    ...
    def test_identity_authorization_uses_only_broker_header(self) -> None:
        self.assertEqual(
        ...
        self.assertIsNone(identity_authorization({"Authorization": "Bearer platform-layer-token"}))
