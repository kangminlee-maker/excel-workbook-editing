...
def evaluate(policy: dict[str, Any], request: dict[str, Any]) -> PolicyDecision:
    ...
    principal_policy = policy.get("principals", {}).get(principal)
    ...
    window_reason = _bounded_window_reason(operation, request, spreadsheet_policy)
    if window_reason:
        return _deny(window_reason, decision_id, version, principal, spreadsheet_id)
